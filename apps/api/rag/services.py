...
Columns: external_id (text), title (text), url (text), topic_tags (text, comma-separated or a single tag, may be NULL for most sources - only populated for GitHub/Hacker News), score (integer - meaning varies by source: GitHub stars, Hacker News points, or a real citation count for the 7 academic sources; 0 where the source has no score concept), published_at (timestamptz), source_id (integer, FK to dim_research_source), source_name (text: 'arxiv', 'github', 'hackernews', 'semantic_scholar', 'openalex', 'crossref', 'dblp', 'hf_papers', or 'zenodo')
Notes: This is the AI & Data Engineering Research pipeline's unified signal feed across 9 real sources, refreshed daily via a self-healing Airflow DAG (ai_dataeng_trends_pipeline). Seven of the nine are genuine research-paper sources (arxiv, semantic_scholar, openalex, crossref, dblp, hf_papers, zenodo) - use source_name IN these 7 (or filter to any/all of them) when a question asks about "research" or "papers" broadly, not only 'arxiv'. github is trending repos (filtered to AI-agent/RAG/MLOps/data-quality/data-mesh topics) and hackernews is real discussions - neither is a paper. Note hf_papers (Hugging Face's Daily Papers) itself draws primarily from arXiv, so meaningful title overlap between hf_papers and arxiv rows is expected and not a data error - hf_papers' score reflects real community upvotes, a different signal than arxiv's own (arxiv rows always have score=0). This is a DIFFERENT, separate pipeline from the older dim_github_repo/fact_github_repo_trend tables (Pipeline 2) - do not conflate the two. For "most cited/most popular" questions among papers, filter to the 7 paper sources and sort by score DESC; for "latest" questions sort by published_at DESC.
